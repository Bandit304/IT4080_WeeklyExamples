...
namespace IT4080C
{
    ...
    {
        public float yDegreesHp;
        public float yDegreesPower;
        public float rSpeed;
        [BurstCompile]
        ...
        public void OnUpdate(ref SystemState state)
        {
                    ...
                    RefRW<HPBox>>().WithEntityAccess().WithAll<Simulate>())
            {
                // Add rotation   
                yDegreesHp += rSpeed;
                Quaternion rRot = Quaternion.Euler(0, yDegreesHp, 0);
            ...
            {
                //rotate object
                yDegreesPower -= rSpeed;
                Quaternion rRot = Quaternion.Euler(0, yDegreesPower, 0);
                localTransform.ValueRW.Rotation = rRot;
                if (state.World.IsServer())
                {
